...
# This script retrieves the current song playing on Spotify, and writes it to a file.
...
CURRENT_SONG_DATED_FILE=$DATADIR/spotify_played_song-$CURRENT_DATETIME.txt
...
echo "$LOG_PREFIX Running get-spotify-now-playing.py to get current song info.";
# capture the output and display
COMMAND="$DIR/venv/bin/python $DIR/get-spotify-now-playing.py --info"
...
if [ -s $CURRENT_SONG_TMP_FILE ]; then
    # if the contents of the tmp file are identical to the current file, skip
    ...
    # ensure the namespaced file exists even if empty
